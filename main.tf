resource "azurerm_network_interface" "interface" {
  ...
  lifecycle {
    precondition {
      condition     = length(each.value.ip_configuration) == 1 && each.value.ip_configuration[0].primary == true
      error_message = "You need to have exactly one IP configuration and it must be marked as primary."
    }
  }
}
